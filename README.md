...
# getAccounttoDataTable.cls

## Overview
The `getAccounttoDataTable` Apex class retrieves a list of `Account` records with selected fields and makes them available for use in a Lightning Web Component (LWC).

## Features
- Retrieves Account data using a SOQL query
- Uses `@AuraEnabled(cacheable=true)` for optimized performance in LWC
- Limits query results to 10 records for efficiency
...
1. Clone the repository:
   ...
   ```
2. Deploy the Apex class to your Salesforce org using Salesforce CLI:
   ```sh
   sfdx force:apex:class:create -n getAccounttoDataTable
   ```
3. Push the class to the org:
   ```sh
   sfdx force:source:push
   ```

## Usage
Import the Apex method in your LWC to fetch Account records:

```javascript
import getAccounttoData from '@salesforce/apex/getAccounttoDataTable.getAccounttoData';
